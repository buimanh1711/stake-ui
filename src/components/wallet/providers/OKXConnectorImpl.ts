 ...
 */
export interface OKXProvider extends OKXEthereumProvider {
  isMetaMask: boolean;
 ...
 */
export interface OKXEthereumProvider {
  isMetaMask?: boolean;
...
export type OKXConnectorOptions = {
  appUrl?: string;
};

export class OKXConnectorImpl extends Connector<
  OKXProvider,
  OKXConnectorOptions
> {
  isActive = true;
  ...
  constructor(options: OKXConnectorOptions = {}) {
    super(options);
    this.check().then(() => {});
  }
  ...
  async connect() {
    ...
    isMulti &&
      (provider =
        provider?.providers?.find((e: OKXProvider) => e.isMetaMask) ||
        provider);
  ...
  async getProvider() {
    if (
      ...
      // @ts-ignore
      return window.okexchain as OKXProvider;
    }
